export default {
  translation: {
    errorMessage: {
      invalidUrl: 'Enter a valid address',
      duplicateUrl: 'This address has already been added to the feed. Enter the new address.',
    },
    requestStatus: {
      success: 'Success. {{statusText}}The channel ({{inputValue}}) successfully added to the list.',
      badRequest: 'Request failed. {{statusText}}The channel ({{inputValue}}) was not added to the list.',
      emptyResponse: 'Empty response. Enter the url carefully.',
    },
    statusRequested: 'requested...',
    defaultValBtnAddChannel: 'Add channel',
  },
};
